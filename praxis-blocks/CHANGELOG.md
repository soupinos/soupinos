...
| 2026-06-03 | `sites/lefkimmi-lines-theme` | SEO pack: keyword H2s (about/why/prices/sched/map); SEO alt texts all images; FAQ section 7 Q&As; FAQPage JSON-LD in head; GDPR cookie banner (localStorage, i18n) + CSS/JS; faq.*/cookie.*/seo.* i18n × 8 langs; updated title keys |
| 2026-06-03 | `sites/lefkimmi-lines-theme` | New WP templates: page-privacy-policy.php (GR+EN GDPR); page-igoumenitsa-kerkyra.php (landing page, comparison table, schedule/booking blocks, FAQPage+BreadcrumbList schema); 3 blog articles (ftino-ploio-kerkyra, notia-kerkyra-odigos, lefkimmi-vs-corfu-port EN) |
| 2026-06-03 | `24-blog-preview` | New: responsive 3-card blog grid, caldera tokens, WP REST API ready, static data-posts fallback, hover animations; added to theme as block24.css/js |
| 2026-06-03 | `sites/lefkimmi-lines-theme` | Pre-launch #1-5: contact form AJAX handler (nonce+honeypot+rate-limit→info@lefkimmilines.gr); favicon.svg; OG+Twitter Card meta; custom 404.php (caldera); dynamic sitemap at /sitemap.xml (WP rewrite); robots.txt updated; sitemap.xml all pages |
| 2026-06-04 | `sites/lefkimmi-lines-theme/assets/app.js` | Fix 1 — Greek uppercase tonos: stripTonosUC() (NFD normalization, keeps dialytika U+0308) + applyGreekCaps() hooks into LLi18n.apply; one-time fix for .lk-txt .ln2 static element |
| 2026-06-04 | `sites/lefkimmi-lines-theme/assets/caldera.css` | Fix 1 — Updated comment: CSS text-transform:uppercase does not strip Greek tonos in Chrome/Blink; JS handles it |
...
| 2026-06-04 | `sites/praxis-therapist/front-page.php` | PHASE 2 — full assembly: hero + #about(20) + #services(25 ×3) + #why(26) + CTA(.book-banner) + #reviews(27, data-render=off until real) + #contact (form id=ll-cform + WhatsApp/email CTAs). GR copy integrated; [placeholders] kept; blocks 20/25/26/27 css/js copied into theme assets; mobile CTA-wrap + .form-err styles. Verified visually (static mirror) |
| 2026-06-17 | `28-px-social` | New: follow links + SEO-aware share row; Web Share API (auto-removes if unsupported) + per-network popups + copy-link; reads canonical/og at click time; light\|dark; caldera tokens |
| 2026-06-17 | `29-px-faq` | New: a11y accordion (one-open, arrow-key nav) + FAQPage JSON-LD built from the rendered DOM; GSAP-height if present else instant; data-schema on\|off; generalised from sites/therapist px-faq |
| 2026-06-17 | `30-px-cta-multi` | New: multiple CTA variants in one block (solid\|outline\|ghost\|banner), data-align, data-surface light\|dark; CSS-only; data-open-modal hook (block #35 listens) |
| 2026-06-17 | `31-px-quick-answer` | New: featured-snippet short-answer card (Q heading + 40–55w answer + more); optional Question/QAPage JSON-LD from rendered text; light\|dark; caldera tokens |
| 2026-06-17 | `32-px-key-facts` | New: figure/label/note grid (2\|3\|4 cols, auto-stack); optional count-up on view (IO, easeOutQuart) with final value in markup; light\|dark |
| 2026-06-17 | `33-px-pros-cons` | New: balanced pros/cons two-column (data-kind pro\|con, auto ✓/— glyphs); meaning carried by title not colour; CSS-only; light\|dark |
| 2026-06-17 | `34-px-nav-mobile` | New: off-canvas drawer (focus trap, Escape, overlay, GSAP slide optional, data-side); CTA **dispatches** px:open-modal — no modal reference; generalised from sites/therapist mobile-nav |
| 2026-06-17 | `35-px-contact-modal` | New: FULL-STACK (HTML+CSS+JS+PHP+README). **Listens** for px:open-modal; desktop popup / mobile bottom-sheet; self-contained --px-modal-* tokens; PHP: SCF on acf/init (guarded, no plugin beyond SCF), honeypot + WP nonce + function_exists() guards, wp_footer render, px_gr_upper() caps. 34↔35 decoupled via event name only |
| 2026-06-17 | `preview/index.html` | Added #28–#35 with labels + TOC dots (sand/dark wrappers); 34→35 wired live so the px:open-modal handoff is clickable in the preview |
