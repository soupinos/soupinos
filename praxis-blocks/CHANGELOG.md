...
|| 2026-06-12 | `28-contact-modal` | New: contact form as modal overlay (z-index 1100, above px-nav at 1000). Listens for `px:open-modal` custom event. Focus trap, Escape close, backdrop click, inline validation, success state. IIFE, zero deps. |
|| 2026-06-12 | `utils/px-modal-bridge.js` | New: delegates `[data-open-modal]` clicks to `px:open-modal` custom events. Drop after any block — connects nav's "Επικοινωνία" link to modal automatically. |
