...
(function (global) {
  ...
  function splitEl(el, mode) {
    ...
    if (mode === 'word') {
      text.split(/(\s+)/).forEach(function (unit) {
        if (unit === '') return;
      ...
      // char mode — split into WORD wrappers first (inline-block + nowrap
      // so a word can never break across lines), then split characters
      // INSIDE each word. The browser may only line-break in the real
      // spaces BETWEEN word wrappers, never mid-word.
